	...
	<div class="course-add-container">
		...
		<el-form :label-position="labelPosition" label-width="100px" ref="courseForm">
							...
							><div class="grid-content ">
								<div class="block date-picker-wrapper">
									<el-form-item
										...
										:class="{
											'is-error':
												$v.startDate.$dirty && !$v.startDate.required,
										}"
									>
										<el-date-picker
											class="date-picker"
          ...
          <el-row :gutter="20">
			...
			<el-button class="succes-btn" type="success submit" @click="handleAddCourse()"
				>Создать</el-button
			>
		</el-form>
	</div>
	...
	methods: {
		...
		handleAddCourse() {
			...
			const formData = {
				...
				start_recruit: this.$options.filters.parseTime(this.startRecruit, '{y}-{m}-{d}'),
				start_date: this.$options.filters.parseTime(this.startDate, '{y}-{m}-{d}'),

<template>
	<div>
		...
		<el-form label-position="top" label-width="100px" class="category-form">
			<el-row :gutter="20" align="bottom" class="category-form-row">
				<el-col :span="6">
					<el-form-item
						...
						:class="{
							'is-error': $v.data.question.$dirty && !$v.data.question.required,
						}"
					>
						<el-input v-model="data.question"></el-input>
						<small
							v-if="$v.data.question.$dirty && !$v.data.question.required"
							class="error-text"
			...
			<el-row :gutter="20" align="bottom" class="category-form-row">
				<el-col :span="12">
					<el-form-item
						...
						:class="{
							'is-error': $v.optionsAnswer.$dirty && !$v.optionsAnswer.required,
						}"
					>
						<div class="answer-field">
							<el-input class="answer-input" v-model="optionsAnswer"></el-input>

							<el-checkbox class="checkbox" v-model="trueAnswerChecked"
								>Правильный вариант ответа</el-checkbox
							>
						</div>

						<small
							v-if="$v.optionsAnswer.$dirty && !$v.optionsAnswer.required"
							class="error-text"
							>Поле "Варианты ответа" не должно быть пустым</small
						>
					</el-form-item>
					<el-button @click="addAnswer" type="success" class="add-category-button"
					...
					<el-table :data="answer" border style="width: 100%">
						<el-table-column label="Варианты ответов">
							<template slot-scope="scope">
								<span :class="scope.row.right_answer ? 'true-answer' : ''">{{
									scope.row.answer
								}}</span>
							</template>
						</el-table-column>
			...
			<el-button type="success" @click="editTestQuestion(data.id)" class="add-category-button"
